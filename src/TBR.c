...
/* function assigns the value for etop if the tree below vtest is moved
 * to be below etop and SPR for tree bottom tree splits ebottom
 * recursive function searching over values of ebottom minor variant of
 * SPR.c's assignUpWeights difference is the index assignment in the
 * array swapWeights, which has a different meaning for the TBR
 * routines also using dXTop to assign value of average distance to tree
 * above vtest SPR performed on tree above vtest... */
	...
	edge *etop, double *bestWeight, edge **bestSplit, edge **bestTop, edge **bestBottom)
{
	...
	if (NULL == etop)
	{
		if (NULL == back)		/* first recursive call */
		{
			...
			{
				assignTBRUpWeights (left, vtest, va, ebottom, va,
								A[va->index][vtest->index], 0.5, A,
								dXTop, swapWeights, NULL, bestWeight,
								bestSplit, bestTop, bestBottom);
				assignTBRUpWeights (right, vtest, va, ebottom, va,
								A[va->index][vtest->index], 0.5, A,
								dXTop, swapWeights, NULL, bestWeight,
								bestSplit, bestTop, bestBottom);
			}
		...
		else	/* NULL != back */
		{
			...
			swapWeights[vtest->index][ebottom->head->index][ebottom->head->index] =
				swapWeights[vtest->index][back->head->index][back->head->index] + (D_AC + D_BD - D_AB - D_CD);
			...
			if (NULL != left)
			{
				assignTBRUpWeights (left, vtest, va, ebottom, sib->head, D_AB, 0.5 * coeff,
						A, dXTop, swapWeights, NULL, bestWeight, bestSplit, bestTop, bestBottom);
				assignTBRUpWeights (right, vtest, va, ebottom, sib->head, D_AB, 0.5 * coeff,
						A, dXTop, swapWeights, NULL, bestWeight, bestSplit, bestTop, bestBottom);
			}
		}
	}
	else	/* NULL != etop */
	{
		if (NULL == back)	/* first recursive call */
		{
			...
			{
				assignTBRUpWeights (left, vtest, va, ebottom, va,
						dXTop[va->index][etop->head->index], 0.5, A, dXTop,
						swapWeights, etop, bestWeight, bestSplit, bestTop, bestBottom);
				assignTBRUpWeights (right, vtest, va, ebottom, va,
						dXTop[va->index][etop->head->index], 0.5, A, dXTop,
						swapWeights, etop, bestWeight, bestSplit, bestTop, bestBottom);
			}
		...
		{
			D_BD = dXTop[ebottom->head->index][etop->head->index];
			/* B is tree above vtest - it is in configuration indexed by etop
			...
			/* it is correct to use A[][] here because the bad average distances
			 * involving B from the first term will be cancelled by the bad average
			 * distances involving B in the subtracted part va is root of subtree
			 * skew back at vtest
			 * A is union of tree below va and all subtrees already passed in path from vtest to ebottom */
			D_AB = 0.5 * (oldD_AB + dXTop[cprev->index][etop->head->index]);
			swapWeights[vtest->index][ebottom->head->index][etop->head->index] =
				swapWeights[vtest->index][back->head->index][etop->head->index] + (D_AC + D_BD - D_AB - D_CD);

			if (swapWeights[vtest->index][ebottom->head->index][etop->head->index] +
				swapWeights[vtest->index][etop->head->index][etop->head->index] < *bestWeight)
				/* first term is contribution of second SPR, second term is contribution of first SPR */
			{
				*bestSplit = vtest->parentEdge;
				*bestTop = etop;
				*bestBottom = ebottom;
				*bestWeight = swapWeights[vtest->index][ebottom->head->index][etop->head->index] +
							swapWeights[vtest->index][etop->head->index][etop->head->index];
			}
			if (NULL != left)
			{
				assignTBRUpWeights (left, vtest, va, ebottom, sib->head,
								D_AB, 0.5*coeff, A, dXTop, swapWeights,
								etop, bestWeight, bestSplit, bestTop, bestBottom);
				assignTBRUpWeights (right, vtest, va, ebottom, sib->head,
								D_AB, 0.5*coeff, A, dXTop, swapWeights,
								etop, bestWeight, bestSplit, bestTop, bestBottom);
			}
		}	/* else NULL != back, etop */
 ...
 * sibling tree of back and D is the tree above vtest
	...
	double *bestWeight, edge **bestSplitEdge, edge **bestTop, edge **bestBottom)
{
	...
	if (NULL == back) /* first recursive call */
	{
		...
		{
	      assignTBRDownWeightsUp (par, vtest, va, etest, va,
		  			A[va->index][vtest->index], 0.5, A, swapWeights,
					bestWeight, bestSplitEdge, bestTop, bestBottom);
	      assignTBRDownWeightsSkew (skew, vtest, va, etest, va,
		  			A[va->index][vtest->index], 0.5, A, swapWeights,
					bestWeight, bestSplitEdge, bestTop, bestBottom);
	    }
	}
	else /* second or later recursive call */
	{
		...
		D_AC = A[sib->head->index][back->head->index] +
				...
				A[sib->head->index][vtest->index]);
		D_AB = 0.5 * (oldD_AB + A[vtest->index][cprev->index]);
		swapWeights[vtest->index][etest->head->index][etest->head->index] =
				swapWeights[vtest->index][back->head->index][back->head->index] +
				(D_AC + D_BD - D_AB - D_CD);
		...
		if (NULL != par)
		{
			assignTBRDownWeightsUp (par, vtest, va, etest, sib->head,
					D_AB, 0.5 * coeff, A, swapWeights, bestWeight,
					bestSplitEdge, bestTop, bestBottom);
			assignTBRDownWeightsSkew (skew, vtest, va, etest, sib->head,
					D_AB, 0.5 * coeff, A, swapWeights, bestWeight,
					bestSplitEdge, bestTop, bestBottom);
		}
	}
...
void assignTBRDownWeightsSkew (edge *etest, node *vtest, node *va,
	edge *back, node *cprev, double oldD_AB, double coeff, double **A,
	double ***swapWeights, double *bestWeight, edge **bestSplitEdge,
	edge **bestTop, edge **bestBottom)
{
	/* same general idea as assignDownWeights, except needing to keep
	 * track of things a bit differently */
	 ...
	 * A is subtree remaining fixed - below va, unioned with all trees
	 * already passed by B
	...
	if (NULL == back)
	{
		...
		{
			assignTBRDownWeightsDown (left, vtest, va, etest, etest->tail,
					A[vtest->index][etest->tail->index], 0.5, A, swapWeights,
					bestWeight, bestSplitEdge, bestTop, bestBottom);
			assignTBRDownWeightsDown (right, vtest, va, etest, etest->tail,
					A[vtest->index][etest->tail->index], 0.5, A, swapWeights,
					bestWeight, bestSplitEdge, bestTop, bestBottom);
		}
	}
	...
	double *bestWeight, edge **bestSplitEdge, edge **bestTop, edge **bestBottom)
{
	...
	if (NULL != left)
	{
		assignTBRDownWeightsDown (left, vtest, va, etest, sib->head,
			D_AB, 0.5 * coeff, A, swapWeights, bestWeight, bestSplitEdge,
			bestTop, bestBottom);
		assignTBRDownWeightsDown (right, vtest, va, etest, sib->head,
			D_AB, 0.5 * coeff, A, swapWeights, bestWeight, bestSplitEdge,
			bestTop, bestBottom);
	}
...
/* vbottom is node below esplit for average calculations in matrix dXTop,
 * A is matrix of average distances from original tree, dXout is average
 * distance from vbottom to tree rooted at far edge of eback, if SPR
 * breaking eback, UpOrDown indicates whether etop is in path above split
 * edge (Up) or not (Down) */
	...
	double dXOut, edge *esplit, edge *etop, edge *eback, int UpOrDown)
{
	...
	/* By moving etop past the vbottom tree, everything in the etop tree
	 * is closer by coefficient of 0.25, while everything in the old back
	 * tree is further by a coefficient of 0.25
	 * Everything in the tree that is being moved (emove) keeps the same
	 * relative weight in the average distance calculation */
...
void TBR (tree *T, double **A, double **D, int *count, FILE *statfile)
{
	...
	double **dXTop;
	/* dXTop[i][j] is average distance from subtree rooted at i to tree
	 * above split edge, if SPR above split edge cuts edge whose head
	 * has index j */
	double bestWeight;
	double treeWeightBefore;
	double ***TBRWeights;
	boolean firstTBR = TRUE;
	...
	TBRWeights = (double ***) mCalloc (T->size, sizeof(double **));
	for (i=0; i<T->size; i++)
	...
	do
	{
		...
		for (esplit = depthFirstTraverse (T, NULL); NULL != esplit; esplit = depthFirstTraverse (T, esplit))
		{
			...
			{
				sib = siblingEdge (esplit);
				/* Next two lines calculate the possible improvements
				 * for any SPR above esplit */
				assignTBRDownWeightsUp (par, esplit->head, sib->head,
						NULL, NULL, 0.0, 1.0, A, TBRWeights, &bestWeight,
						&eBestSplit, &eBestTop, &eBestBottom);
				assignTBRDownWeightsSkew (sib, esplit->head, sib->tail,
						NULL, NULL, 0.0, 1.0, A, TBRWeights, &bestWeight,
						&eBestSplit, &eBestTop, &eBestBottom);

				calcTBRaverages(T,esplit,A,dXTop);
				/* Calculates the average distance from any subtree
				 * below esplit to the entire subtree above esplit,
				 * after any possible SPR above */

				/* for etop above esplit, we loop using information from
				 * above to calculate values for all possible SPRs below
				 * esplit */
			}
			...
			if (NULL != right)
			{
				...
				assignTBRUpWeights (left, esplit->head, right->head,
						NULL, NULL, 0.0, 1.0, A, dXTop, TBRWeights,
						NULL, &bestWeight, &eBestSplit, &eBestTop, &eBestBottom);
				assignTBRUpWeights (right, esplit->head, left->head,
						NULL, NULL, 0.0, 1.0, A, dXTop, TBRWeights,
						NULL, &bestWeight, &eBestSplit, &eBestTop, &eBestBottom);
				...
				while (NULL != block)
				{
					if (block != esplit)
					{
						...
						assignTBRUpWeights (left, esplit->head, right->head,
								NULL, NULL, 0.0, 1.0, A, dXTop,TBRWeights,
								etop, &bestWeight, &eBestSplit, &eBestTop, &eBestBottom);
						assignTBRUpWeights (right, esplit->head, left->head,
								NULL, NULL, 0.0, 1.0, A, dXTop, TBRWeights,
								etop, &bestWeight, &eBestSplit, &eBestTop, &eBestBottom);
					}
		...
		if (bestWeight < -FLT_EPSILON)
		{
			...
			if (firstTBR)
			{
				...
				{
					if (verbose > 2)
						Debug ("Before TBR: tree length is %f.", treeWeightBefore);
					else if (verbose > 1)
						Message (". Before TBR: tree length is %f.", treeWeightBefore);
				}
			}

			if (verbose > 2 && !isBoostrap)
			{
				if ((NULL == eBestTop->label) || (strlen (eBestTop->label) == 0))
				{
					if ((NULL == eBestBottom->label) ||(strlen (eBestBottom->label) == 0))
						Debug ("Slitting edge '%s'.", eBestSplit->label);

					else
						Debug ("Slitting edge '%s' on top edge and bottom edge '%s'.",
							eBestSplit->label, eBestBottom->label);
				}
				else
				{
					if ((NULL == eBestBottom->label) || (strlen (eBestBottom->label) == 0))
						Debug ("Splitting edge '%s' on top edge '%s' and bottom edge.",
							eBestSplit->label, eBestTop->label);

					else
						Debug ("Splitting edge '%s' on top edge '%s' and bottom edge '%s'.",
							eBestSplit->label, eBestTop->label, eBestBottom->label);
				}
				Debug ("Old tree length is %f, new tree length should be %f.",
			...
			if (!isBoostrap)
			{
				fprintf (statfile, "\tTBR  %5d: new tree length is %f.\n", *count, T->weight);
				if (verbose > 2)
					Debug ("TBR %5d: new tree length is %f.", *count, T->weight);
				else if (verbose > 1)
					Message (". TBR %5d: new tree length is %f.", *count, T->weight);
			}
		}
		else
			bestWeight = 1.0;

	} while (bestWeight < -FLT_EPSILON);
...
/* General idea is to have a double loop for a given edge,
 * testing all SPRs for the subtrees above and below a given edge.
